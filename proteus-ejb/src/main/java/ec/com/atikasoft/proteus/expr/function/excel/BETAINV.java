...
extends AbstractFunction {
    ...
    public Expr evaluate(IEvaluationContext context, Expr[] args) throws ExprException {
        ...
        Expr eBeta = BETAINV.evalArg(context, args[2]);
        ...
        double b = 1.0;
        if (args.length > 3) {
            Expr eA = BETAINV.evalArg(context, args[3]);
            if (!this.isNumber(eA)) {
                return ExprError.VALUE;
            }
        ...
        if (args.length > 4) {
            Expr eB = BETAINV.evalArg(context, args[4]);
            if (!this.isNumber(eB)) {
                return ExprError.VALUE;
            }
