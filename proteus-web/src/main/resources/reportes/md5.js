...
function validar () {    
    ...
    //read the url values
    user=request.getParameter("p_usu");
    repname=request.getParameter("__report");
    urlstring=md5(repname+user);

    //set the flag to allow the report components to hide if flag=1 (= no access)
    if (cookiestring!=urlstring)
    {
         reportContext.getDesignHandle().findElement("tabla_Proteus").drop();
    } else {
         reportContext.getDesignHandle().findElement("mensaje_Proteus").drop();
    }
}
...
function restarFechas(finicio, ffin){
      if(finicio == null || ffin == null){
        return;
      }
      var d = finicio; //establecemos la fecha de hoy
...
function nombreMeses(numeroMes){ 
var mes; 
switch (row["mesfechaInicio"]){ 
case 1:mes="ENERO"; 
break; 
case 2:mes="FEBRERO"; 
break; 
